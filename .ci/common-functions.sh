...
annotate-manifest-for-tag() {
    ...
    for ARCH in ${SUPPORTED_ARCHITECTURES}; do
        if [[ ${DOCKER_OFFICIAL} = true ]]; then
            docker_official_uri=$(echo ${DOCKER_REGISTRY}/${ARCH}/${DOCKER_IMAGE} | sed 's/^\/*//')
            docker_manifest_command="docker manifest annotate ${docker_uri}:${manifest_tag} ${docker_official_uri}:${manifest_tag} --arch ${ARCH}"
        fi
        
        if [[ ${DOCKER_OFFICIAL} = false ]]; then
            docker_manifest_command="docker manifest annotate ${docker_uri}:${manifest_tag} ${docker_uri}:${ARCH}-${manifest_tag} --arch ${ARCH}"
        fi
    done
    echo ${docker_manifest_command}
}
